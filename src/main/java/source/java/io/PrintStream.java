 ...
 *
 1.PrintStream向另一个输出流添加功能，即方便地打印各种数据值的表示的能力。
 2.还提供了另外两个功能。 与其他输出流不同， PrintStream从不抛出IOException ；
 相反，异常情况只是设置一个可以通过checkError方法测试的内部标志。
 3.可选地，可以创建PrintStream以自动刷新；
 这意味着在写入字节数组、调用println方法之一或写入换行符或字节 ( '\n' ) 后，会自动调用flush方法。
 PrintStream打印的所有字符都使用平台的默认字符编码转换为字节。 PrintWriter类应该用于需要写入字符而不是字节的情况
 * @author     Frank Yellin
    ...
    implements Appendable, Closeable
{
     ...
     * 跟踪文本和字符输出流，以便可以在不刷新整个流的情况下刷新它们的缓冲区
     */
     ...
     * 这里明确声明了 requireNonNull 以免创建对 java.util.Objects.requireNonNull 的额外依赖。
     * PrintStream 在系统初始化期间提前加载。
     */
     ...
     * or the equivalent.
     * 刷新流并检查其错误状态。
     * 当底层输出流抛出InterruptedIOException以外的IOException并且调用setError方法时，
     * 内部错误状态设置为true 。
     * 如果对底层输出流的操作抛出InterruptedIOException ，则PrintStream通过执行以下操作将异常转换回中断：
     *            Thread.currentThread().interrupt();
     *
     * 或等价物。
     ...
     * methods.
     * 将指定的字节写入此流。 如果字节是换行符并且启用了自动刷新，则将调用flush方法。
     * 请注意，字节是按给定的方式写入的；
     ...
     * encoding, use the <code>print(char)</code> or <code>println(char)</code>
     * methods.
     * 将指定字节数组中的len个字节从偏移量off开始写入此流。 如果启用了自动刷新，则将调用flush方法。
     * 请注意，字节将按照给定的方式写入；
     * 要编写将根据平台默认字符编码进行翻译的字符，请使用print(char)或println(char)方法。
     ...
     * are written in exactly the manner of the
     * <code>{@link #write(int)}</code> method.
     * 打印一个对象。 String.valueOf(Object)方法生成的字符串根据平台默认的字符编码转换为字节，
     * 这些字节的写入方式与write(int)方法完全相同。
     ...
     * <code>{@link #println()}</code>.
     * 打印一个 long 然后终止该行。 这个方法的行为就像它先调用print(long)然后调用println()
     ...
     * <code>{@link #println()}</code>.
     * 打印一个 double 然后终止该行。 此方法的行为就像它先调用print(double)然后调用println()
     ...
     * though it invokes <code>{@link #print(String)}</code> and then
     * <code>{@link #println()}</code>.
     * 打印一个对象，然后终止该行。 此方法首先调用 String.valueOf(x)
     * 以获取打印对象的字符串值，然后其行为就像调用print(String)和println() 。
     ...
     * 例如，调用字符缓冲区的 then toString方法将返回一个子序列，其内容取决于缓冲区的位置和限制
     * @param  csq
